...
def test_model_performance(model_name, stage, holdout_data_path, vectorizer_path):
    try:
        ...
        X_holout_tfidf_df = pd.DataFrame(X_holdout_tfidf.toarray(), columns=vectorizer.get_feature_names_out())

        # Predict using the model
        y_pred_new = model.predict(X_holout_tfidf_df)
        ...
        assert precision_new > expected_precision, f"Accuracy should be at least {expected_precision}, got {precision_new}"    
